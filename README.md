...
- **語音筆記（選用）** — 用麥克風錄音，或直接上傳音檔／影片（最大 500 MB），交由後端（`backend/`）處理音檔並透過 Groq 轉成逐字稿，再由 Gemini 自動整理成一篇結構化筆記。面板可以縮小到背景，轉錄與生成完成後會自動建立筆記並提示，不需要一直盯著等待。原始錄音與逐字稿會完整保留在本機瀏覽器（IndexedDB）裡以便日後核對，設定頁面可查看目前佔用空間並隨時清除。
...
想啟用語音筆記功能，除了 Gemini 金鑰之外，還需要在同一個設定頁面貼上 [Groq API 金鑰](https://console.groq.com/keys)（同樣只存在瀏覽器裡）。點擊頂欄的麥克風圖示即可開始錄音，停止後會自動上傳並轉錄成逐字稿、生成一篇新筆記。也可以直接上傳音訊或**影片**檔案，最大 500 MB（影片會由後端自動抽出音軌再轉錄，無需另外轉檔）。錄音時可以選擇同時擷取分享分頁的聲音（例如視訊會議對話）並保留畫面影像；錄好或轉錄失敗時，都能直接下載原始檔案留存。

語音筆記功能需要 `backend/` 底下的音檔處理後端服務同時執行（負責音檔正規化、切割與呼叫 Groq），本機開發時另開一個終端機執行：
