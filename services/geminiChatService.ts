...
export const createChatSession = async (
    ...
    try {
        ...
        const systemInstruction = language === 'en'
            ? 'You are an AI learning assistant. A user has provided you with their notes. Your role is to help them understand, summarize, or quiz them on the provided content in a conversational manner. Be helpful and encouraging. Always respond in English. When your answer involves a mathematical formula, chemical equation, or a symbolic logical relationship (e.g. "A leads to B" written as A → B), write it in LaTeX: wrap inline formulas in a single `$` (e.g. `$\\rightarrow$`, `$x^2$`) and standalone block formulas in `$$` (e.g. `$$E=mc^2$$`). LaTeX commands always use a single backslash (e.g. `\\rightarrow`, `\\frac{a}{b}`) — never double it. Only use this syntax when actual math or symbolic notation is involved.'
            : 'You are an AI learning assistant. A user has provided you with their notes. Your role is to help them understand, summarize, or quiz them on the provided content in a conversational manner. Be helpful and encouraging. Always respond in Traditional Chinese (繁體中文). 當回答涉及數學公式、化學方程式或需要符號呈現的邏輯關係（例如「A 導致 B」可寫成 A → B）時，請使用 LaTeX 語法撰寫：行內公式用單一 `$` 包住（如 `$\\rightarrow$`、`$x^2$`），獨立成行的公式用 `$$` 包住（如 `$$E=mc^2$$`）。LaTeX 指令一律使用單一反斜線（如 `\\rightarrow`、`\\frac{a}{b}`），不要重複跳脫。沒有數學或符號需求時不要濫用這個語法。';
...
export const generateNoteFromTranscript = async (transcript: string, apiKey: string): Promise<string> => {
    ...
    try {
        ...
        const contents = language === 'en'
            ? `Please organize the following speech transcript into a well-structured note. Use "#"/"##" headings to divide topics, and "-" bullet lists to present key points, preserving the original meaning and important details while removing filler words, repetition, and off-topic content. Headings alone are enough to separate sections — do not insert "---" horizontal rule dividers between them. When the transcript mentions a mathematical formula, chemical equation, or a symbolic logical relationship (e.g. "A leads to B" written as A → B), write it in LaTeX: wrap inline formulas in a single \`$\` (e.g. \`$\\rightarrow$\`, \`$x^2$\`) and standalone block formulas in \`$$\` (e.g. \`$$E=mc^2$$\`). LaTeX commands always use a single backslash (e.g. \`\\rightarrow\`, \`\\frac{a}{b}\`) — never double it. Only use this syntax when actual math or symbolic notation is involved. Regardless of what language the transcript itself is in, write the note entirely in English (proper nouns, technical terms, code, etc. that can't be translated accurately may stay in their original form). Output only the Markdown note itself, with no explanatory text or Markdown code block markers.\n\nTranscript:\n${transcript}`
            : `請將以下語音逐字稿整理成一份結構清楚的筆記。使用「#」「##」等標題劃分主題，並用「-」列表呈現重點，保留原意與重要細節，並移除口語贅字、重複與離題內容。標題本身就足以區隔段落，不要在段落之間插入「---」之類的分隔線。若逐字稿提到數學公式、化學方程式或需要符號呈現的邏輯關係（例如「A 導致 B」可寫成 A → B），請使用 LaTeX 語法撰寫：行內公式用單一 \`$\` 包住（如 \`$\\rightarrow$\`、\`$x^2$\`），獨立成行的公式用 \`$$\` 包住（如 \`$$E=mc^2$$\`）。LaTeX 指令一律使用單一反斜線（如 \`\\rightarrow\`、\`\\frac{a}{b}\`），不要重複跳脫。沒有數學或符號需求時不要濫用這個語法。不論逐字稿本身是什麼語言，筆記內容一律使用繁體中文撰寫（人名、專有名詞、程式碼等無法翻譯或翻譯後會失真的內容可保留原文）。只輸出 Markdown 筆記本身，不要加上任何說明文字或 Markdown 程式碼區塊符號。\n\n逐字稿：\n${transcript}`;
        const systemInstruction = language === 'en'
            ? 'You are an expert note-taker. Convert raw speech transcripts into well-organized Markdown notes with clear headings and bullet points, preserving the original meaning and key details without adding commentary. Always write the note in English, regardless of what language the transcript itself is in — proper nouns, technical terms, and code may stay in their original form when translating them would be inaccurate or lose meaning. Do not insert horizontal rule dividers ("---") between sections — headings alone are enough to separate them. When math or symbolic notation is involved, write it in LaTeX with a single backslash per command (e.g. `\\rightarrow`, never `\\\\rightarrow`) — inline formulas wrapped in a single `$`, block formulas in `$$`.'
            : 'You are an expert note-taker. Convert raw speech transcripts into well-organized Markdown notes with clear headings and bullet points, preserving the original meaning and key details without adding commentary. Always write the note in Traditional Chinese (繁體中文), regardless of what language the transcript itself is in — proper nouns, technical terms, and code may stay in their original form when translating them would be inaccurate or lose meaning. Do not insert horizontal rule dividers ("---") between sections — headings alone are enough to separate them. 涉及數學或符號時請用 LaTeX 語法，指令一律用單一反斜線（如 `\\rightarrow`，不要寫成 `\\\\rightarrow`）——行內公式用單一 `$` 包住，獨立成行則用 `$$` 包住。';
