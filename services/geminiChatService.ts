...
export const generateNoteFromTranscript = async (transcript: string, apiKey: string): Promise<string> => {
    ...
    try {
        ...
        let response;
        try {
            response = await generate('gemini-3.6-flash');
        } catch (error) {
            ...
            console.warn('Gemini 3.6 Flash is overloaded; retrying note generation with 3.5 Flash.');
            try {
                ...
                // recover when 3.6 Flash becomes available again.
